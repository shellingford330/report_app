...
class MultiReportsController < ApplicationController
  before_action :teacher_logged_in, only: %i[index new create edit_multi]

  # 指導報告書の生徒選択
  def index
    @students_id = if (students_id = params[:students_id])
                     students_id.map(&:to_i)
                   else
                     []
                   end
  end
  ...
  # 指導報告書の複数新規作成のまとめて編集
  def edit
    @students_id = params[:report][:student_id].split.map(&:to_i)
    @reports = []
    @students_id.each do |student_id|
