...
class Teacher < ApplicationRecord
  ...
  after_destroy do
    logger.info("Teacher is deleted: #{attributes.inspect}")
  end
  ...
  validates :status,
            inclusion: { in: %w[teacher manager], if: proc { |teacher| teacher.changed == ["status"] } }
  validates :password,
            presence: true,
             length: { minimum: 6, allow_nil: true },
             allow_nil: true
  validate do |teacher|
    errors.add(:image, "5MB以下にして下さい。") if teacher.image.size > 5.megabytes
  end
